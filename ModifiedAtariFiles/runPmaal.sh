...
SwarmbotGazeboDQN="/root/torch-ros-gazebo/swarmbotgazebo-dqn" # to put in environment variable!!
...
# ( roslaunch swarm_simulator soup.launch gui:=false ) &  # doesn't work well yet, so commented for now
...
echo "---GOTTOSETUP"
th $SwarmbotGazeboDQN/setup.lua $args
#Load program to allocate rewards
echo "--GOTTOGNOME2"
setup_command="th $SwarmbotGazeboDQN/rewards.lua $args"
gnome-terminal -e "bash -c \"$setup_command ; exec bash\""
# ( th $SwarmbotGazeboDQN/rewards.lua $args ) &
echo "---GOTTOMAIN"
th main.lua -threads $NUM_BOTS \
   -zoom 4 \
   -env swarmbotgazebo-dqn/GazeboEnv \
   -modelBody swarmbotgazebo-dqn/SwarmbotModel \
   -histLen 4 \
   -async A3C \
   -entropyBeta 0.001 \
   -eta 0.0007 \
