...
function ValidationAgent:validate()
  ...
  local valTotalScore = 0
	...
	print('ValidationAgent 2 - agent:validate()')
  ...
  for valStep = 1, self.valSteps do
    ...
    if not terminal then
      ...
      valEpisode = valEpisode + 1
			--print('ValidationAgent 3 - terminal reached')
  ...
  self:visualiseFilters()
	...
	--Turn this off for now since it crashes it
...
--Functions added in to cut down memory usage
-------------------------------------------------------------------
...
function ValidationAgent:plotValidation()
  ...
  -- Plot and save V
	local space = 8
  local epochIndices = linspace(#self.avgV, space)
  local Vs = torch.Tensor(self.avgV)
	local Vs2 = spaced_data(epochIndices, #self.avgV, space, Vs)
	--This line crashes it
  gnuplot.pngfigure(paths.concat('experiments', self._id, 'Vs.png'))
  gnuplot.plot('V', epochIndices, Vs2, '-')
  ...
  local scores = torch.Tensor(self.valScores)
	local scores2 = spaced_data(epochIndices, #self.avgV, space, scores)
  gnuplot.pngfigure(paths.concat('experiments', self._id, 'scores.png'))
  gnuplot.plot('Score', epochIndices, scores2, '-')
  gnuplot.xlabel('Epoch')
...
function ValidationAgent:linspace(sample_size, space)
	x = torch.Tensor(torch.floor(sample_size / space))
	for i=1, torch.floor(sample_size / space) do
		x[i] = space * i
	end
	return x
...
function ValidationAgent:spaced_data(linspace, sample_size, space, data)
	y = torch.Tensor(torch.floor(sample_size / space))
	for i=1, torch.floor(sample_size / space) do
		y[i] = data[x[i]]
	end
	return y
